...
const OurWork: FunctionComponent<OurWorkType> = ({ className = "" }) => {
  ...
  const handlePrevious = useCallback(() => {
    setSelectedImageIndex((prevIndex) => 
      prevIndex !== null ? (prevIndex > 0 ? prevIndex - 1 : images.length - 1) : null
    );
  }, []);

  const handleNext = useCallback(() => {
    setSelectedImageIndex((prevIndex) => 
      prevIndex !== null ? (prevIndex < images.length - 1 ? prevIndex + 1 : 0) : null
    );
  }, []);
    ...
    if (visibleImages < images.length) {
      ...
      setTimeout(() => {
        ...
        if (container) {
          ...
          if (newImageElement) {
            newImageElement.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'start' });
          }
        }
      }, 100);
    }
  ...
  return (
    ...
    >
      <h2 className="w-full text-center capitalize font-medium text-[52px] mb-2">Our Work</h2>
      ...
      >
        {images.slice(0, visibleImages).map((item, index) => (
          <div key={index} className="flex flex-col items-center w-[200px] flex-shrink-0 snap-start">
